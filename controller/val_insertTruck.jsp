	...
	<%
		...
		String contentType = request.getContentType();
      	...
      	if ((contentType != null) && (contentType.indexOf("multipart/form-data") >= 0)) {
            ...
            image = image.substring(0,image.indexOf("\""));
            path = "C:/xampp/tomcat/webapps/WebProg/Project_Kelas/img/" + image;
		...
		if(truckBrand.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyTruckBrand");
		}
		else if(truckType.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyTruckType");
		}
		else if(truckYear.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyTruckYear");
		}
		else if(truckPlate.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyTruckPlate");
		}
		else if(stnkNumber.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyStnkNumber");
		}
		else if(stnkYear.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyStnkYear");
		}
		else if(stnkExpiredDate.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyStnkExpiredDate");
		}
		else if(driverName.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyDriverName");
		}
		else if(image.length()<1){
			response.sendRedirect("../insertTruck.jsp?code=emptyImage");
		}
		else{
			Truck truck = new Truck();
			if(truck.exists("truckPlate",truckPlate)!=null){
				response.sendRedirect("../insertTruck.jsp?code=truckExist");
			}
			else{
				...
				truck.insert();
				response.sendRedirect("../insertTruck.jsp?code=insertSuccess");
			}
